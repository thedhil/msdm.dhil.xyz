  ...
  const updateStatus = (
    ...
    setTimeout(() => {
      if (listType === "internal") {
        setInternal((prev) =>
          prev.map((c) => (c.name === name ? { ...c, status: newStatus } : c))
        );
      } else {
        setExternal((prev) =>
          prev.map((c) => (c.name === name ? { ...c, status: newStatus } : c))
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-1">
                <Button
                  ...
                  onClick={() =>
                    updateStatus(listType, candidate.name, "accepted")
                  }
                >
                ...
                <Button
                  ...
                  disabled={loading === candidate.name}
                  onClick={() =>
                    updateStatus(listType, candidate.name, "review")
                  }
                >
                  {loading === candidate.name ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                ...
                <Button
                  ...
                  disabled={loading === candidate.name}
                  onClick={() =>
                    updateStatus(listType, candidate.name, "pending")
                  }
                >
                  {loading === candidate.name ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Admin dashboard for internal committee and external professional
          approval
        </p>
      </div>
